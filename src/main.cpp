...
const string strMessageMagic = "Stakers Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 99)
    ...
    else if (pindexBest->nHeight <= 9999999)
      {
        int64_t nSubsidy = 0 * COIN;
        return nSubsidy + nFees;
      }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Stakers", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
